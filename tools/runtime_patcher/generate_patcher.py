...
1. Launches Dead Space 2
2. Waits for activation.x86.dll to be loaded and unpacked
3. Patches the SSL verification in memory

This works around the DLL protection by patching after unpacking.
"""

import os
import sys

# Generate C code for the patcher
PATCHER_CODE = r'''
/*
 * Dead Space 2 Runtime SSL Patcher
 * 
...
def main():
    output_path = os.path.join(os.path.dirname(__file__), 'ds2_ssl_patcher.c')
    ...
    print()
    print("To compile on Linux (cross-compile for Windows):")
    print("  i686-w64-mingw32-gcc -o ds2_ssl_patcher.exe ds2_ssl_patcher.c -lpsapi")
    print()
    print("To compile on Windows with MinGW:")
    print("  gcc -o ds2_ssl_patcher.exe ds2_ssl_patcher.c -lpsapi")
    print()
    print("To compile on Windows with MSVC:")
    print("  cl ds2_ssl_patcher.exe ds2_ssl_patcher.c /link psapi.lib")
    print()
    print("Usage:")
    print("  1. Start Dead Space 2")
    print("  2. Run ds2_ssl_patcher.exe (as Administrator)")
    print("  3. Wait for 'Successfully applied X patches'")
    print("  4. Go to multiplayer in the game")
